                        ...
                        requestFields(
                                fieldWithPath("loginId").type(JsonFieldType.STRING)
                                        ...
                                        .attributes(key("validity").value(
                                                "로그인 아이디는 5자리 ~ 20자리 사이입니다. 로그인 아이디는 영문(대소문자 구분)과 숫자만 허용합니다.")),
                                fieldWithPath("password").type(JsonFieldType.STRING)
                                        .description("패스워드")
                                        .attributes(key("validity").value(
                                                "비밀번호는 8자리 ~ 20자리 사이 입니다. 비밀번호는 특수문자를 반드시 포함하여야 합니다.")),
                                fieldWithPath("email").type(JsonFieldType.STRING)
                                        .description("이메일")
                                        .attributes(key("validity").value(
                                                "이메일은 최소 3자리 부터 최대 25자리 까지 입니다. 이메일 형식을 지켜야 합니다.")),
                                fieldWithPath("username").type(JsonFieldType.STRING)
                                        .description("회원 이름")
                                        .attributes(key("validity").value(
                                                "이름은 2글자 ~ 15글자 사이입니다. 이름은 한글과 영문만 허용됩니다."))
                        ),
                        responseFields(
    ...
    void signup2() throws Exception {
        ...
        mockMvc.perform(post("/user/signup")
                ...
                .andExpect(jsonPath("$.isSuccess").value(
                        ErrorCode.DUPLICATED_SIGNUP_DATA.getIsSuccess()))
                .andExpect(
                        jsonPath("$.message").value(ErrorCode.DUPLICATED_SIGNUP_DATA.getMessage()))
